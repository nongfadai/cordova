...
<ul class="wx_bank_list">
    ...
    <li class="<%if(!data.idNO){%>wx_bank_list_input<%}%>"><span>身份证号码</span>
    	<%if(data.idNO){%>
        	<i><%=data.idNO%></i>
        <%}else {%>
        	<div class="wx_bank_auth_box"><input class="input-text-2 input-no-icon" type="tel" value="" maxlength="18" id="idNo" name="idNo"  placeholder="请输入您的身份证号" />
    ...
    <li class="wx_bank_auth_tip <%=liClass%>"><span>开户银行</span>
        ...
        <%}else {%>
        	...
        	<div class="wx_bank_auth_box"><input class="input-text-2 input-no-icon input-select" type="text" value="选择银行" id="bankName" name="bankName" readonly="readonly" /></div>
