				...
				success: function(d){
					if(d){
						...
						for(var i = 0;i<len;i++){
							...
							var icon = "";
							if(creditInfo.F32==1){
								icon = "<i class=\"n_product_icon icon_new\">新手</i>";
							}else{
								if (creditInfo.F30 > 0 && creditInfo.F31 != 'PTB') {
									icon = "<i class=\"n_product_icon icon_best\">优</i>";
								} else {
									if (creditInfo.F16 == 'S') {
										icon = "<i class=\"n_product_icon icon_bao\">保</i>";
									} else if (creditInfo.F17 == 'S') {
										icon = "<i class=\"n_product_icon icon_bao\">抵</i>";
									} else if (creditInfo.F18 == 'S') {
										icon = "<i class=\"n_product_icon icon_bao\">实</i>";
									} else {
										icon = "<i class=\"n_product_icon icon_bao\">信 </i>";
									}
								}
							}
							morehtml.push("<a href=\"project_details.html?id="+creditInfo.F02+"\" class=\"project "+project_status+"\">");
							morehtml.push("<div class=\"title\">");
							morehtml.push("<span class=\"name\">"+creditInfo.F04+icon+"</span>");
							...
							if(creditInfo.F11 != creditInfo.YFB) {
								if(creditInfo.proess==1){
									morehtml.push("<span class=\"btn btn_gray\">"+btnName+"</span>");
								}else{
									morehtml.push("<span class=\"btn\">"+btnName+"</span>");
								}
							}else{
							...
							morehtml.push("</div>");
							morehtml.push("</div>");
							morehtml.push("<div class=\"right progress progress-bar-"+parseInt(creditInfo.proess100)+"\">");
							morehtml.push("<div class=\"percent\">");
							if(creditInfo.proess==1){
								morehtml.push("满");
							}else{
								morehtml.push(parseInt(creditInfo.proess100)+"<span class=\"secondary\">%</span>");
							}
							morehtml.push("</div>");
							morehtml.push("</div>");
							morehtml.push("</div>");
